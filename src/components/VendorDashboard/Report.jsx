...
const SummaryCard = ({ title, value, icon, color, loading }) => {
  ...
  return (
    <div
      className={`p-6 rounded-2xl text-white shadow-lg bg-gradient-to-br ${colors[color]}`}
    >
...
const ProfitLossCard = ({ title, value, loading }) => {
  ...
  return (
    <div
      className={`p-6 rounded-2xl shadow-lg ${bgColor} border ${
...
const CustomerTable = ({
  ...
  const handleEditClick = (customerId) => {
    ...
    // Or link to the receipt page with the customer pre-loaded:
    // navigate(`/receipt?customerId=${customerId}`);
    toast.info(
      `Edit button clicked for customer ID: ${customerId}. You need to add navigation logic here.`
    );
  };
          ...
          {customers.map((c, index) => {
            ...
            return (
              ...
              >
                ...
                <td className="p-3 whitespace-nowrap text-sm text-green-600 font-medium text-right">
                  {formatCurrency(yene)}
                </td>
  ...
  if (error && allCustomers.length === 0) {
    return (
      <div className="bg-white rounded-2xl shadow-xl p-6 max-w-7xl mx-auto">
        <EmptyState
          icon={<AlertCircle className="text-red-500 text-3xl" />}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-4 print-hidden">
              <div className="relative">
                ...
                <input
                  ...
                  className="border border-gray-300 rounded-lg py-2 pl-10 pr-4 w-full focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
              </div>
                  ...
                  {allCustomers.map((c) => {
                    ...
                    return (
                      <tr key={c._id}>
                        ...
                        <td className="p-3 whitespace-nowrap text-sm text-green-600 text-right">
                          {formatCurrency(yene)}
                        </td>
