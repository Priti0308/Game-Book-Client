...
// Helper function to safely evaluate arithmetic expressions for display purposes
                        ...
                        {(receiptData.gameRows || []).map((row, index) => {
                             ...
                             const renderCellWithCalculation = (colName) => {
                                 ...
                                 return (
                                     <div className="flex flex-col items-end p-1">
                                         <div className="w-full text-center border-b border-gray-400 pb-1 mb-1">{row[colName]}</div>
                                         {hasMultiplier && (
                            ...
                            return (
                                <tr key={row.id || index}>
                                    ...
                                    <td className="border border-black p-1 text-center">{`${row.pan.val1 || ''} × ${row.pan.val2 || ''} = ${(toNum(row.pan.val1) * toNum(row.pan.val2)).toFixed(0)}`}</td>
                                    <td className="border border-black p-1 text-center">{`${row.gun.val1 || ''} × ${row.gun.val2 || ''} = ${(toNum(row.gun.val1) * toNum(row.gun.val2)).toFixed(0)}`}</td>
                                </tr>
                            );
                        })}
                         ...
                         <tr><td className="border border-black p-1">टो.</td><td className="border border-black p-1 text-right font-bold">{toNum(receiptData.totalDue).toFixed(2)}</td><td colSpan="5" className="border border-black p-1"></td></tr>
...
const ViewReceipts = () => {
  ...
  const handleEdit = (receiptId) => {
    // Navigate to the create receipt page, passing the ID to edit
